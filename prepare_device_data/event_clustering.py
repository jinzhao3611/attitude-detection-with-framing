...
import operator
from data.get_event_lemmas import get_non_single_trigger_lemma, RESERVED_LEMMAS
...
def merge_cluster(clusters, cluster_pairs, epoch, topics_counter,
    ...
    for i, cluster_pair in enumerate(cluster_pairs):
        # if i % 5000 == 0:
        #     print('Cluster pairs to merge {}/{}'.format(i, len(cluster_pairs)))
...
def generate_cluster_pairs(clusters, is_train):
    ...
    for i, cluster_1 in enumerate(clusters):
        # if i % 100 == 0:
        #     print('Cluster {}/{}'.format(i, len(clusters)))
        ...
        for t in data:
            for doc_name in data[t]:
                ...
                for sen_id in doc.sentences.keys():
                    ...
                    event_mentions.extend([e for e in sen.events if e.lemma == topic])
...
def write_mention_based_cd_clusters(corpus, is_event, is_gold, out_file, dataset, news_topic):
    '''
    ...
    reserved_lemmas = RESERVED_LEMMAS[news_topic]
        ...
        for doc_id in doc_keys:
            ...
            for sen_id in sorted(cur_doc.sentences.keys()):
                ...
                for mention in mentions:
                    ...
                    # coref_chain = mention.cd_coref_chain
                    if dataset == 'ecb':
                        if mention.lemma in reserved_lemmas:
                            out_coref.write('{}\t({})\n'.format(mention.mention_id, mention.lemma))
                        else:
...
def write_event_coref_results(corpus, out_dir, isGold, dataset, alpha, threshold, news_topic):
    # if not isGold:
    ...
    out_file = os.path.join(out_dir, f'cluster_{alpha}_{threshold}.txt')
    # write_mention_based_cd_clusters(corpus, is_event=True, is_gold=False, out_file=out_file, dataset=dataset)
    write_mention_based_cd_clusters(corpus, is_event=True, is_gold=False, out_file=out_file, dataset=dataset,
                                    news_topic=news_topic)


def test_models(dataset, test_set, write_clusters, out_dir, isProcessed, alpha, threshold, news_topic):
    '''
    ...
    if dataset == 'ecb':
        topics = get_non_single_trigger_lemma(news_topic)  # list of lemmas
        # topics = [list(list(test_set.values())[0].keys())]  # list of lists

    topics_num = len(topics)
    print('Number of topics: {}'.format(topics_num))
    ...
    with torch.no_grad():
        for i, topic in enumerate(topics):
            ...
            print('Number of clusters = {}'.format(len(topic_event_clusters)))

            cluster_pairs, _ = generate_cluster_pairs(topic_event_clusters, is_train=False)
            print('Number of cluster pairs: {}'.format(len(cluster_pairs)))

            merge_cluster(topic_event_clusters, list(cluster_pairs), epoch, topics_counter, topics_num, threshold,
                          alpha, True)
            print("Merging done!")
            print('Number of clusters: {}'.format(len(topic_event_clusters)))
        ...
        if write_clusters:
            write_event_coref_results(test_set, out_dir, True, dataset, alpha, threshold, news_topic)
...
@click.command()
@click.option('--news_topic', type=click.STRING)
def main(news_topic: str):
    model_out_file = TOPIC_NAME_MAPPING[news_topic] / "pairwise_scores.txt"
    logger.info(f'write {model_out_file} to {TOPIC_NAME_MAPPING[news_topic]}')

    test_data = prepare_ecb_docs(news_topic)
    test_data = {news_topic: test_data}  # make it in the same format with ECB docs input

    logger.info('Test data have been loaded.')

    alpha = [0.8]

    threshold = [0.5]  # use one threshold for now
    ...
    for i in alpha:
        for j in threshold:
            ...
            test_models('ecb', test_data, write_clusters=True, out_dir=TOPIC_NAME_MAPPING[news_topic], isProcessed=True,
                        alpha=i, threshold=j, news_topic=news_topic)
